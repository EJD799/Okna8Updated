var Settings = {
    ...
    YourAccount: /*html*/ `
        ...
        <br>
        <button class="MetroButton2" style="margin-left:0" onclick="sendToTop('ModalMetroDialog|' + LOCALE_app_settings[6]['YourAccount'][8].replace('username', localStorage.getItem('OKNA8_user_' + currentUser + '_username')))">${LOCALE_app_settings[6]['YourAccount'][4]}</button>
        <br><br>
    ...
    WindowsUpdate: /*html*/ `
        <script>
            function checkupdates () {
                $('#WindowsUpdate-ProgressRing').css('display','inline-block')
                $('#WindowsUpdate-CheckButton').css('display','none')
                $('#WindowsUpdate-UpdateButton').css('display','none')
                $('#WindowsUpdate-P').html('')
                axios.post(VERSION.server + '/okna/GetUpdates', '{"ver":' + VERSION.build + '}').then(response=>{
                    if (response.data.success) {
                        var updateFile = response.data.updateFile
                        $('#WindowsUpdate-ProgressRing').css('display','none')
                        $('#WindowsUpdate-CheckButton').css('display','inline-block')
                        if (updateFile.LatestVer > VERSION.build) {
                            $('#WindowsUpdate-UpdateButton').css('display','inline-block')
                            $('#WindowsUpdate-UpdateButton').attr('onclick', 'window.open("' + updateFile.LatestVerDownload + '")')
                            $('#WindowsUpdate-P').html("${LOCALE_app_settings[6]['WindowsUpdate'][3]}" + updateFile.LatestVerName)
                        } else {
                            $('#WindowsUpdate-P').html("${LOCALE_app_settings[6]['WindowsUpdate'][4]} (" + VERSION.ver + ')')
                        }
                    }
                })
            }
        ...
        <button id="WindowsUpdate-CheckButton" class="MetroButton2" style="margin-left:0" onclick="checkupdates()">${LOCALE_app_settings[6]['WindowsUpdate'][2]}</button>
        <button id="WindowsUpdate-UpdateButton" class="MetroButton2" style="display:none" onclick="checkupdates()">${LOCALE_app_settings[6]['WindowsUpdate'][5]}</button>
    `,
    Recovery: /*html*/ `
