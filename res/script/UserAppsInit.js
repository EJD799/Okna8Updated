...
function ConnectMetroApp(UserApp) {
    if (VERSION["build"] >= UserApp[5]) {
        ...
        // 0 - package, 1 - name (label), 2 - locale, 3 - color, 4 - color2, 5 - min.ver, 6 - target ver, 7 - tile sizes (array), 8 - appver, 9 - livetile sizes (array), 10 - app path dir

        var AppPath = '../../../../mods/apps/' + UserApp[0]
        if (UserApp[10] != null && UserApp[10] != '') {
            AppPath = UserApp[10] + UserApp[0]
        }
    
        LOCALE_appsnames.push(UserApp[1])
        LOCALE_searchdata.push([UserApp[0], UserApp[7], UserApp[3], AppPath, UserApp[1]])
        tilesInfo[UserApp[0]] = [UserApp[7], UserApp[3], LOCALE_appsnames.length - 1, AppPath, UserApp[1], UserApp[3], UserApp[4], UserApp[9]]

        if (typeof updateAppsContainer != "undefined") updateAppsContainer
    } else {
        ModalMetroDialog(`
