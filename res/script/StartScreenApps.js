    ...
    function add() {
        if (i < SortedAppsList.length) {
            ...
            if (SortedAppsList[i][1] == 'DESKTOP') {
                ...
                var icon
                if (tilesInfo[SortedAppsList[i][1]][5].startsWith('data:image')) {
                    icon = tilesInfo[SortedAppsList[i][1]][5]
                } else {
                    icon = desktopRes + tilesInfo[SortedAppsList[i][1]][5]
                }
                $(category + '<div class="appInAppscontainer DesktopApp" id="appInAppsContainer1_desktop_' + SortedAppsList[i][1].substring(8) + '" onclick="Exec(\'' + SortedAppsList[i][1].substring(8) + '\');closeStart()"><div class="iconbackground" style="background-color:' + tilesInfo[SortedAppsList[i][1]][2] + ';"></div><img id="appInAppsContainer2_desktop_' + SortedAppsList[i][1].substring(8) + '" src="' + icon + '"><p id="appInAppsContainer3_desktop_' + SortedAppsList[i][1].substring(8) + '">' + SortedAppsList[i][0] + '</p></div>' + category2).appendTo('.appscontainer')
            } else {
