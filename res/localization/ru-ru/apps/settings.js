var LOCALE_app_settings = [
    ...
    {                                   // Содержание Параметров
        ...
        'YourAccount':[
            ...
            'Обзор',
            'Удалить',
            '<h1>Изменение имени пользователя</h1><p>Можно использовать любые буквы, цифры, пробелы и символы. <br><br>Текущее имя пользователя: username</p><input class="MetroDialogPcRenameInput" type="text"><div class="buttons"><button onclick="localStorage.setItem(\'OKNA8_user_\' + currentUser + \'_username\', $(\'.MetroDialogPcRenameInput\').val());$(\'#metrowindow_Settings iframe\')[0].contentWindow.postMessage(\'UserNameChanged\', \'*\');CloseMetroDialog(__ID__)">Далее</button><button onclick="CloseMetroDialog(__ID__)">Отмена</button></div>',
            '<h1>Переименуйте компьютер</h1><p>После перезапуска имя компьютера изменится на: pcname</p><div class="buttons"><button onclick="shutdown(\'r\');CloseMetroDialog(__ID__)">Перезагрузить сейчас</button><button onclick="CloseMetroDialog(__ID__)">Перезагрузить позже</button></div>'
        ],
        'WindowsUpdate':[
            'Центр обновления Windows',
